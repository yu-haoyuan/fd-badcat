...
INTERRUPT_LIMIT = int(1.5 / FRAME_SEC)   # 1.5s 打断计数阈值
END_HOLD_SEC = 0.64
...
def stream_vad_asr_pipeline(audio_path: str, output_dir: str):
    ...
    for frame in stream_audio(audio_path):
        ...
        #     log.write(f"{event}\n")
        #     log.write(f"[{state}] Media Time: {media_time:.2f}s\n")
        #     print(f"[{state}] Media Time: {media_time:.2f}s, Event: {event}", file=log)
        # ============ LISTEN 状态 ============
        if state == "LISTEN":
        ...
        # ============ SPEAK 状态 ============
            ...
            # sys_start, sys_finish = current_sys_range
            # if not event or "start" not in event:
            #     if sys_start is not None and media_time >= sys_finish:
            #         state = "LISTEN"
            #         in_speech = False
            #         silence_counter = 0
            #         continue
...
def main():
    ...
    #wav_path = Path("exp/exp1/dev/dev_zh/") #
    wav_path = Path("/home/sds/data/dev/dev_zh/Follow-upQuestions")
    #output_root = Path("exp/exp1/medium/medium_zh/")
    output_root = Path("/home/sds/output")

    wav_files = get_time_wav(wav_path)
    print(f"找到 {len(wav_files)} 个 .wav 文件进行处理")
    for wav in tqdm(wav_files):
        wav_file = wav_path / wav  
        tqdm.write(f"处理: {wav_file}")

        output_dir = output_root / wav_path.stem / wav_file.stem
        output_dir.mkdir(parents=True, exist_ok=True) 

        stream_vad_asr_pipeline(str(wav_file), str(output_dir))
        wav_files_in_dir = list(Path(output_dir).glob("*.wav"))
        json_files_in_dir = list(Path(output_dir).glob("*.jsonl"))
