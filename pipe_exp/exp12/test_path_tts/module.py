...
def target_speaker(ref_audio: np.ndarray, user_audio: np.ndarray):
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as f1, \
        tempfile.NamedTemporaryFile(suffix=".wav", delete=True) as f2:
        sf.write(f1.name, ref_audio, 16000)
        sf.write(f2.name, user_audio, 16000)
        result = sv_model([f1.name, f2.name])
    return result["score"]
...
def main():
    ...
    ref_path = "model/0001_0002_a1.wav"
    user_path = "model/0001_0002_a2.wav"
    ref_audio, sr1 = sf.read(ref_path, dtype="float32")
    user_audio, sr2 = sf.read(user_path, dtype="float32")
    result = target_speaker(ref_audio, user_audio)
    print(result)
