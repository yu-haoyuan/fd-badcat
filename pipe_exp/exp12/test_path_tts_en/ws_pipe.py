...
from module import asr, llm_qwen3o, tts, target_speaker
import argparse
import uvicorn
import yaml
import copy
...
class ConversationEngine:
    def __init__(self, websocket: WebSocket = None, prompts: dict = None, delay: dict = None):
        ...
        self.sv_score = float(delay["sv_score"])
            ...
            if event and "start" in event:
                ...
                if elapsed_silence >= self.END_HOLD_FRAMES:
                    ...
                    if self.ref_audio is None:
                        self.ref_audio = user_audio.copy()
                    else:
                        spk_score = target_speaker(self.ref_audio, user_audio)
                        await self.send_control("speaker_verification", {
                            "timestamp": round(time.time() - self.start_wall, 3),
                            "score": float(spk_score),
                            "turn": self.TURN_IDX
                        })
                        if spk_score < self.sv_score:
                            system_repeat = await self.async_llm(self.SV_PROMPT, None, self.TURN_IDX, add_to_history=True)
                            asyncio.create_task(self.async_tts(system_repeat, self.TURN_IDX))
                            return
