...
python pipe_exp/exp13/test_path_tts/ws_f.py --config pipe_exp/exp13/test_path_tts/config.yaml

#注意：ws_f.py 是前端脚本，执行完毕后会退出（打印 “全部完成”）。
# 此时后端 ws_pipe.py 仍在后台运行。

# ---------- 4. 执行 inter_score.py ----------
python pipe_exp/exp13/test_path_tts/inter_score.py --config pipe_exp/exp13/test_path_tts/config.yaml
